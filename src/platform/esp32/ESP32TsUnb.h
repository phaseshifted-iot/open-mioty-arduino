...
class ESP32TsUnb {
public:
    ESP32TsUnb() : preciseTsUnbTimer(0.0f), timerHandle(nullptr), transmissionActive(false) {
    }
     ...
     */
    IRAM_ATTR void addTimerDelay(const int32_t count) {
     ...
     * 
     * Safe to call multiple times - ensures correct pin configuration.
     * On Nesso N1, this ensures custom pins (SCK=20, MISO=22, MOSI=21) are used.
     * 
     ...
     * 
     * NOTE: Does NOT call SPI.end() - SPI bus may be shared with display.
    ...
    /**
     * @brief Check if a mioty transmission is currently active
     * 
     * This flag is set at the start of transmit() and cleared at the end.
     * User code (buttons, sensors, display) should check this before
     * performing time-consuming operations to avoid interfering with
     * critical mioty timing.
     * 
    ...
    //! Transmission active flag (volatile for ISR safety) - public for driver access
